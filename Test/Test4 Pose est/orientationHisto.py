...
# Calculate the overall mean and standard deviation for angle_diff
mean_all = np.mean(angle_diff)
std_all = np.std(angle_diff)
...
for i in range(len(bins) - 1):
    ...
    if np.any(bin_mask):
        bin_values = angle_diff[bin_mask]
...
bars = plt.bar(bin_centers, mean_values, width=10, edgecolor='black', alpha=1, label='Angle Error Mean', color='cornflowerblue')
...
plt.ylabel('Angle Error Mean [degrees]', fontsize=20)
plt.title('Mean Angle Error over time without Vicon correction', fontsize=20)
plt.legend()
