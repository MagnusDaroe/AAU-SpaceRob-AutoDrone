...
DATADIR = "_Calibrate_camera_2\Img_for_calibration_2\\"
...
def get_board_pos(image,board,mtx,dist):
    ...
    if marker_ids is not None and len(marker_ids) > 0: # If at least one marker is detected
        ...
        ret, charucoCorners, charucoIds = cv2.aruco.interpolateCornersCharuco(marker_corners, marker_ids, cv2.cvtColor(image, cv2.COLOR_BGR2GRAY), board)
    ...
    return tvec,np.array(all_charuco_corners)
    ...
    while stop==True:
        ...
        if left_frame:          
            ...
            if len(t_vec_)>2:
                ...
                t_vec_=t_vec_*100
                t_vec_flat=t_vec_.round(0).flatten().astype(int)
                ...
                print("t_vec_ - x: {}, y: {}, z: {}".format(t_vec_[0][0],t_vec_[1][0],t_vec_[2][0]))
