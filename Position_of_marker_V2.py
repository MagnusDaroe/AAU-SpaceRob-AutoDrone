...
def get_T_matrix_q(qrot_xyzw,translation_xyz):
    ...
    """
    x_t=translation_xyz[0]*100 #convert to cm
    y_t=translation_xyz[1]*100 #convert to cm
    z_t=translation_xyz[2]*100 #convert to cm
    ...
    y = qrot_xyzw[1]
    z = qrot_xyzw[2]
    T = np.array([[-(1-2*y*y-2*z*z), 2*x*y-2*z*w, -(2*x*z+2*y*w),x_t],
