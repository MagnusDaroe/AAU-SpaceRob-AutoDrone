...
calib_imgs_path = root.joinpath("Calibrate_camera\Img_for_calibration")#root.joinpath("aruco_data")
...
aruco_dictionary = cv2.aruco.getPredefinedDictionary(aruco.DICT_6X6_1000)
...
markerLength = 5.45  # Here, measurement unit is centimetre.

# Provide separation between markers
markerSeparation = 0.7   # Here, measurement unit is centimetre.
...
if calibrate_camera == True:
    ...
    for im in tqdm(img_list):
        ...
        #corners, ids, rejectedImgPoints = detector.detectMarkers(img_gray, aruco_dict, parameters=arucoParams)
        if first == True:
