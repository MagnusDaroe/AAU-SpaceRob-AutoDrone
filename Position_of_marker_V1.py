...
try:
    ...
    while stop==True:
        ...
        if left_frame:          
            ...
            T_new=new_get_T_matrix_q(rotation_xyzw,translation_xyz)

            R_q=get_R_matrix_q(rotation_xyzw)
            R_new=new_get_R_matrix_q(rotation_xyzw,translation_xyz)

            
            start_pos=inv(R_q)@(np.array([[0], [0], [-10]]))
            start_pos=start_pos+np.array([[translation_xyz[0]],[translation_xyz[1]],[translation_xyz[2]]])*100
            print("start_pos: ",start_pos)
            start_pos_int=start_pos.astype(int)
            cv2.putText(image_rgb, str("x: {}, y: {}, z: {}".format(start_pos_int[0],start_pos_int[1],start_pos_int[2])),(20, 40), cv2.FONT_HERSHEY_SIMPLEX,0.5, (0, 255, 0), 2)
